...
import numpy as np
import pytest
from tfcg_charcoal.core import Parameters,legacy_growth,corrected_growth,parameter_draws,simulate
from tfcg_charcoal.landscape import slope_degrees

def test_corrected_matches_frost_equivalent_age_below_K():
    ...
    np.testing.assert_allclose(corrected_growth(b,k,q),expected,rtol=1e-7)

def test_growth_domain_and_upper_stock_rule():
    b=np.array([0,.1,1,4.9,5,6,7],dtype=np.float32)
    out=corrected_growth(b,5,.82)
    assert out[0]==0
    assert np.all(out[1:4]>b[1:4]) and np.all(out[1:4]<5)
    np.testing.assert_array_equal(out[4:],b[4:])
    for q in [-.1,0,1,1.1,np.nan]:
        with pytest.raises(ValueError):corrected_growth(b,5,q)

def test_legacy_bug_counterexamples_are_preserved_only_in_legacy():
    assert legacy_growth(np.array([1],np.float32),5,.95)[0]<1
    assert corrected_growth(np.array([1],np.float32),5,.95)[0]>1
    assert legacy_growth(np.array([6,7],np.float32),5,.82)[0]>7
    np.testing.assert_array_equal(corrected_growth(np.array([6,7],np.float32),5,.82),[6,7])
...
def test_skipped_plot_waits_until_next_rotation_and_mass_balance():
    ...
    captured=[]
    totals,diag=simulate(np.array([.5],np.float32),np.array([True]),np.array([2015]),np.array([True]),p,parameter_draws(p,7),mode='corrected',capture=lambda j,t,b,post,h:captured.append((b.copy(),post.copy(),h.copy())),compact=False)
...
def test_threshold_equality_and_exclusion():
    p=replace(Parameters(),realizations=1,rotations=1)
    r,d=simulate(np.array([1,3],np.float32),np.array([True,True]),np.array([2015,2015]),np.array([True,False]),p,parameter_draws(p,7),mode='corrected')
    assert r[0,0,1]==pytest.approx(.9,abs=1e-7)
    assert r[0,0,2]==pytest.approx(.18,abs=1e-7)

def test_compaction_preserves_both_engines():
    ...
    draws=parameter_draws(p,10,bounded=True)
    for mode in ['legacy','corrected']:
        a,da=simulate(b,mask,years,eligible,p,draws,mode=mode,compact=True)
        z,dz=simulate(b,mask,years,eligible,p,draws,mode=mode,compact=False)
...
def test_draws_are_reproducible_and_bounded():
    ...
    a=parameter_draws(p,8,True);b=parameter_draws(p,8,True)
...
def test_invalid_ensemble_configuration_is_rejected():
    for p in [replace(Parameters(),realizations=0),replace(Parameters(),rotations=0),replace(Parameters(),q_mean=np.nan)]:
        with pytest.raises(ValueError):parameter_draws(p,1,True)
